...
export type LogoutDeps = { revoke: (sessionId: string) => Promise<void>; clearCookies: () => void; logError?: (message: string) => void };

export function logoutTarget(
  session: { authMode: "v2" | "legacy"; sessionId: string | null } | null,
): LogoutTarget {
  if (session?.authMode === "v2" && session.sessionId) return { mode: "v2", sessionId: session.sessionId };
  return { mode: "none" };
}
...
export async function performLogout(target: LogoutTarget, deps: LogoutDeps): Promise<{ ok: true }> {
  ...
  try {
    if (target.mode === "v2") {
      try { await deps.revoke(target.sessionId); }
      catch (error) { revokeFailed = true; deps.logError?.(`[protebe] logout/revoke: ${error instanceof Error ? error.message : "failed"}`); }
    }
  } finally { deps.clearCookies(); }
