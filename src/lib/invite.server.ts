...
export async function revokeOpenPartnerInvites(pairId: string, now: Date): Promise<void> {
  ...
  const { error } = await supabase
    ...
    .eq("pair_id", pairId)
    .eq("target_role", INVITE_TARGET_ROLE)
    .is("consumed_at", null)
    .is("revoked_at", null);
...
export async function createPartnerInvite(input: {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      link_secret_digest: issued.linkSecretDigest,
      manual_code_hmac: issued.manualCodeHmac,
...
export async function claimInvite(input: {
  ...
  const { data, error } = await supabase
    .from("pair_invites")
    .update({ consumed_at: now.toISOString(), consumed_by_session_id: input.sessionId })
    .eq("id", input.inviteId)
    .is("consumed_at", null)
    .is("revoked_at", null)
