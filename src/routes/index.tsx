...
function LoveCounter({ anniversary }: { anniversary: string }) {
  ...
  return (
    <GlassCard
      strong
      className="anim-rise p-5 text-center sm:p-6"
      aria-label={`${duration.future ? "Zbývá do našeho dne" : "Spolu už"} ${duration.days} dní`}
    >
      ...
      <p className="mt-3 font-display text-[3.5rem] font-light leading-none veil-text tabular-nums sm:text-6xl">
        {duration.days}
      </p>
      ...
      <div className="mt-5 grid grid-cols-3 gap-2 text-center">
        {[
          ...
          <div key={unit.label} className="rounded-2xl bg-secondary/40 px-2 py-2.5 sm:px-3">
            <p className="font-display text-lg text-foreground tabular-nums sm:text-xl">
...
function Dashboard() {
  ...
  return (
    <div className="space-y-5 pb-4 sm:space-y-6">
      ...
      <section className="px-4 sm:px-6" aria-label="Počítadlo našeho času">
        ...
        <section className="px-4 sm:px-6" aria-labelledby="invite-heading">
          <GlassCard className="anim-fade-up p-4 sm:p-5">
            ...
            <Link
              to="/nastaveni"
              preload="intent"
              className="tap mt-4 flex min-h-11 w-full items-center justify-between rounded-2xl bg-primary/12 px-4 py-3 text-sm font-medium text-primary transition-colors hover:bg-primary/16 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background"
            >
      ...
      <section className="space-y-3 px-4 sm:px-6" aria-label="Přehled">
        {events.isLoading ? (
        ...
        <Link to="/vzkazy" preload="intent" className="block">
          <GlassCard className="p-4 sm:p-5">
            <div className="flex items-center justify-between">
              ...
              <MessageCircleHeart size={16} className="text-primary" aria-hidden="true" />
            </div>
            {messages.isLoading ? (
              <Shimmer className="mt-3 h-6 w-3/4" />
            ) : (
              <p className="mt-3 line-clamp-3 font-display text-lg font-light leading-snug text-foreground sm:text-xl">
