  ...
  server: {
    handlers: {
      GET: async ({ params }) => {
        ...
        if (!/^[0-9a-f-]{36}$/i.test(pairId) || !isCalendarKey(calendarKey))
        ...
        const { data: pair, error } = await supabaseAdmin
          ...
          .maybeSingle();
        if (error) {
          console.error(`[protebe] calendar/pair: ${error.message}`);
        ...
        const lines: string[] = [
          "BEGIN:VCALENDAR",
          "VERSION:2.0",
          "PRODID:-//Pro Tebe//Nas kalendar//CS",
          "CALSCALE:GREGORIAN",
          "METHOD:PUBLISH",
        ...
        if (pair.anniversary) {
          lines.push(
            "BEGIN:VEVENT",
            `UID:anniversary-${pairId}@protebe`,
            `DTSTAMP:${stamp(new Date())}`,
            `DTSTART;VALUE=DATE:${compactDate(pair.anniversary)}`,
            `DTEND;VALUE=DATE:${compactDate(addDays(pair.anniversary, 1))}`,
            "RRULE:FREQ=YEARLY",
            `SUMMARY:${escapeIcs(`Náš den — ${pair.owner_name} & ${pair.partner_name}`)}`,
            "TRANSP:TRANSPARENT",
            "END:VEVENT",
          );
        }
        ...
        for (const event of events ?? []) {
          ...
          if (event.all_day || !event.starts_at) {
            lines.push(
              `DTSTART;VALUE=DATE:${compactDate(event.starts_on)}`,
              `DTEND;VALUE=DATE:${compactDate(addDays(event.starts_on, 1))}`,
            );
          } else {
            const time = String(event.starts_at).slice(0, 5).replace(":", "");
            lines.push(
              `DTSTART;TZID=Europe/Prague:${compactDate(event.starts_on)}T${time}00`,
              "DURATION:PT1H30M",
            );
          }
