...
export const Route = createFileRoute("/nastaveni")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Jména, náš den, připojení a vzhled aplikace — vše na jednom místě.",
      },
      ...
      <section className="px-6">
        <GlassCard className="p-5">
          ...
          <p className="mt-3 text-xs text-muted-foreground">
            {pair.has_partner
              ? "Jste propojení. Vidíte oba to samé, změny se objeví hned."
              : canInvite
                ? "Vytvoř jednorázový kód a pošli jí ho. Platí 24 hodin."
                : "Nový kód pro připojení může vytvořit jen zakladatel prostoru."}
          </p>
          <div className="mt-3 flex items-center gap-2 text-xs text-muted-foreground">
            <span
              className={`size-2 rounded-full ${partnerOnline ? "bg-primary" : connected ? "bg-muted-foreground/60" : "bg-destructive"}`}
            />
            {partnerOnline
              ? "Právě je tady s tebou"
              : connected
                ? "Živé propojení je aktivní"
                : "Připojuji živé propojení…"}
          </div>
        </GlassCard>
      </section>
      <section className="space-y-3 px-6">
        ...
        <button
          ...
          onClick={() => {
            setLeaving(true);
            leave()
              .then(() => toast.success("Odhlášeno z tohoto zařízení"))
        ...
        <p className="px-1 text-[11px] leading-relaxed text-muted-foreground">
          Data zůstanou zachovaná. Toto zařízení bude odpojeno.
        </p>
      </section>
    </div>
  );
...
function InviteCard() {
  ...
  const create = useMutation({
    mutationFn: () => createFn({}),
    onSuccess: (next) => setTicket(next),
    onError: (error) =>
      toast.error(error instanceof Error && error.message ? error.message : INVITE_INVALID_MESSAGE),
  });
  const revoke = useMutation({
    mutationFn: (inviteId: string) => revokeFn({ data: { invite_id: inviteId } }),
    onSuccess: () => {
      setTicket(null);
      toast.success("Kód byl zrušen");
    },
    onError: () => toast.error("Kód se nepodařilo zrušit."),
  });
  function copy(value: string, message: string) {
    navigator.clipboard
      ?.writeText(value)
      .then(() => toast.success(message))
      .catch(() => toast.error("Kopírování se nepovedlo"));
  }
  return (
      ...
      {ticket ? (
        <>
          ...
          <p className="text-[11px] text-muted-foreground">
            Platí 24 hodin. Použije se jen jednou.
          </p>
          <button
            onClick={() =>
              copy(`${window.location.origin}${ticket.deep_link}`, "Odkaz na připojení zkopírován")
            }
            className="tap w-full rounded-2xl border border-border/60 px-4 py-2.5 text-xs text-foreground"
          >
            Zkopírovat odkaz na připojení
          </button>
          <div className="flex gap-2">
            <button
              disabled={revoke.isPending}
              onClick={() => revoke.mutate(ticket.invite_id)}
              className="tap flex-1 rounded-2xl border border-border/60 px-4 py-2.5 text-xs text-muted-foreground disabled:opacity-60"
            >
              Zrušit kód
            </button>
            <button
              ...
              className="tap flex-1 rounded-2xl border border-border/60 px-4 py-2.5 text-xs text-foreground disabled:opacity-60"
            >
        ...
        <PrimaryButton loading={create.isPending} onClick={() => create.mutate()}>
          Vygenerovat nový kód
        </PrimaryButton>
      )}
    </div>
